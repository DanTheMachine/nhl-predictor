...
- Live stats from ESPN replace the hardcoded `gf`/`ga`/`cf`/`xgf`/`pdo`/`sv` baselines when fetched
...
Tests live in `src/**/*.test.{ts,tsx}`. Run with `npm test`.

- `engine.ts` has no dedicated test file — logic is covered indirectly through export and AnalysisPanel tests
- `evaluation.test.ts` contains CSV fixture strings; the parser handles both old (`HOME ML`) and new (`COL ML`) label formats
- `export.test.ts` tests `buildExportRow` and `rowsToCSV`
- `AnalysisPanel.test.tsx` tests edge-strength thresholds and O/U consistency

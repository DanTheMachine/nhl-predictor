...
export function buildExportRow(row: LinesRow, liveStats?: Record<string, LiveTeamStats>): ExportRow {
  ...
  return {
    ...
    mlValueSide: analysis ? (analysis.mlValueSide === "none" ? "PASS" : `${analysis.mlValueSide === "home" ? game.homeAbbr : game.awayAbbr} ML`) : "—",
    mlKelly: analysis && analysis.mlValueSide !== "none" ? `${(Math.max(analysis.kellyHome, analysis.kellyAway) * 100).toFixed(1)}%` : "—",
    puckLineRec: analysis ? (analysis.puckLineRec === "pass" ? "PASS" : analysis.puckLineRec.replace(/^home/, game.homeAbbr).replace(/^away/, game.awayAbbr).toUpperCase()) : "—",
